...
function buildSourceCode(input, language) {
    ...
    if (shouldUseTypescriptParser(language)) {
        ...
        try {
            return (0, parsers_1.parseForESLint)(input.fileContent, vueFile ? parsers_1.parsers.vuejs.parse : parsers_1.parsers.typescript.parse, (0, parsers_1.buildParserOptions)(options, false));
        }
        catch (error) {
    ...
    let moduleError;
    try {
        return (0, parsers_1.parseForESLint)(input.fileContent, vueFile ? parsers_1.parsers.vuejs.parse : parsers_1.parsers.javascript.parse, (0, parsers_1.buildParserOptions)({ parser: vueFile ? parsers_1.parsers.javascript.parser : undefined }, true));
    }
    catch (error) {
